...
public sealed class QueryRewriteService : IQueryRewriteService
{
    ...
    {
        _logger = logger;
        _enabled = configuration.GetValue<bool?>("Features:EnableQueryRewrite") ?? true;

        if (!_enabled)
        {
            _logger.LogInformation("Query rewrite is disabled by configuration (Features:EnableQueryRewrite=false).");
            return;
        }
